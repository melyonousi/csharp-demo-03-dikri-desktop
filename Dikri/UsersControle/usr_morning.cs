...
namespace Dikri
{
    public partial class usr_morning : UserControl
    {
        public usr_morning()
        {
            InitializeComponent();
        ...
        private void usr_morning_Load(object sender, EventArgs e)
        {
            try
            {
                mornings = Morning.Upload(mornings);

                string[] splt = mornings[0].ToString().Split(',');
        ...
        private void btn_next_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (num == mornings.Count)
                    return;

                string lst = string.Format("{0}", mornings[num]);
        ...
        private void btn_previous_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string lst = string.Format("{0}", mornings[numLeft]);
                string[] splt = lst.Split(',');
                lbl_number.Text = $"{int.Parse(splt[0]).ToString("D2")}:{Program.dikrNumber}";
                btn_count.Text = $"{int.Parse(splt[1]).ToString("D2")}";
                btn_count.Tag = $"{int.Parse(splt[1]).ToString("D2")}";
