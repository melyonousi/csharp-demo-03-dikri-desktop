...
namespace Dikri
{
    public partial class usr_sleep : UserControl
    {
        public usr_sleep()
        {
            InitializeComponent();
        ...
        private void usr_sleep_Load(object sender, EventArgs e)
        {
            try
            {
                sleeps = Sleep.Upload(sleeps);

                string[] splt = sleeps[0].ToString().Split(',');
        ...
        private void btn_next_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string lst = string.Format("{0}", sleeps[num]);
        ...
        private void btn_previous_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string lst = string.Format("{0}", sleeps[numLeft]);
                string[] splt = lst.Split(',');
                lbl_number.Text = $"{int.Parse(splt[0]).ToString("D2")}:{Program.dikrNumber}";
                btn_count.Text = $"{int.Parse(splt[1]).ToString("D2")}";
                btn_count.Tag = $"{int.Parse(splt[1]).ToString("D2")}";
